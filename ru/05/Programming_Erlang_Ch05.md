...
## Binaries. Бинарные последовательности. ##
...
Эта функция инверсна функции term_to_binary:
...
<http://media.pragprog.com/titles/jaerlang/code/mp3_sync.erl>
...
find_sync пытается найти три последовательных заголовка MPEG фреймов.Если байт N в Bin является началом заголовка фрейма, то is_header(N, Bin) вернет {ok, Length, Info}. Если is_header возврашает error. то N
...
данных графических ресурсов в бинарную последовательность:
	...
	unpack_image_resource_directory(Dir) ->
		...
		NumberOfIdEntries : ?WORD, _binary>> = Dir,
		...
...
* Выражения и последовательности выражений: Что именно представляет из
себя выражение?
...
Атрибуты модуля имеют синтаксис -AtomTag(...) и используются для
...
<http://media.pragprog.com/titles/jaerlang/code/abc.erl>
...
<http://media.pragprog.com/titles/jaerlang/code/extract.erl>
...
Внутри определения макроса поддерживаются следующие директивы. Вы можете
...
Числа с плавающей точкой имеют пять частей: необязательный знак, целая
